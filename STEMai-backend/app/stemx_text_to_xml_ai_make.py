...
def _add_value_with_colour_picker(block_el: ET.Element, name: str, hex_colour: str) -> None:
    """Adds <value name=...><shadow type='colour_picker'><field name='COLOUR'>#RRGGBB</field></shadow></value>"""
    val_el = ET.SubElement(block_el, "value", {"name": name})
    shadow_el = ET.SubElement(val_el, "shadow", {"type": "colour_picker", "id": _new_id()})
    field_el = ET.SubElement(shadow_el, "field", {"name": "COLOUR"})
    field_el.text = hex_colour
...
def _make_block(cmd: BlockCmd) -> ET.Element:
    ...
    # ----- Sensing (NEW) -----
    if cmd.kind == "sensing_touchingobject":
        block = ET.Element("block", {"type": "sensing_touchingobject", "id": _new_id()})
        tgt = _normalize_target(cmd.target)
        _add_value_with_menu(block, "TOUCHINGOBJECTMENU", "sensing_touchingobjectmenu", "TOUCHINGOBJECTMENU", tgt)
        return block

    if cmd.kind == "sensing_touchingcolor":
        block = ET.Element("block", {"type": "sensing_touchingcolor", "id": _new_id()})
        col = (cmd.text or "#000000").strip()
        if col and not col.startswith("#") and re.fullmatch(r"[0-9a-fA-F]{6}", col):
            col = "#" + col
        if not col.startswith("#"):
            col = "#000000"
        _add_value_with_colour_picker(block, "COLOR", col)
        return block

    if cmd.kind == "sensing_answer":
        return ET.Element("block", {"type": "sensing_answer", "id": _new_id()})

    if cmd.kind == "sensing_coloristouchingcolor":
        block = ET.Element("block", {"type": "sensing_coloristouchingcolor", "id": _new_id()})
        c1 = (cmd.text or "#000000").strip()
        c2 = (cmd.text2 or "#000000").strip()
        if c1 and not c1.startswith("#") and re.fullmatch(r"[0-9a-fA-F]{6}", c1): c1 = "#" + c1
        if c2 and not c2.startswith("#") and re.fullmatch(r"[0-9a-fA-F]{6}", c2): c2 = "#" + c2
        _add_value_with_colour_picker(block, "COLOR", c1)
        _add_value_with_colour_picker(block, "COLOR2", c2)
        return block

    if cmd.kind == "sensing_distanceto":
        block = ET.Element("block", {"type": "sensing_distanceto", "id": _new_id()})
        tgt = _normalize_target(cmd.target)
        _add_value_with_menu(block, "DISTANCETOMENU", "sensing_distancetomenu", "DISTANCETOMENU", tgt)
        return block

    if cmd.kind == "sensing_askandwait":
        block = ET.Element("block", {"type": "sensing_askandwait", "id": _new_id()})
        _add_value_with_text(block, "QUESTION", cmd.text or "What's your name?")
        return block

    if cmd.kind == "sensing_keypressed":
        block = ET.Element("block", {"type": "sensing_keypressed", "id": _new_id()})
        key = (cmd.key or "space").strip()
        _add_value_with_menu(block, "KEY_OPTION", "sensing_keyoptions", "KEY_OPTION", key)
        return block

    if cmd.kind == "sensing_mousex":
        return ET.Element("block", {"type": "sensing_mousex", "id": _new_id()})
    if cmd.kind == "sensing_mousey":
        return ET.Element("block", {"type": "sensing_mousey", "id": _new_id()})
    if cmd.kind == "sensing_mousedown":
        return ET.Element("block", {"type": "sensing_mousedown", "id": _new_id()})

    if cmd.kind == "sensing_setdragmode":
        block = ET.Element("block", {"type": "sensing_setdragmode", "id": _new_id()})
        field_el = ET.SubElement(block, "field", {"name": "DRAG_MODE"})
        field_el.text = cmd.style or "draggable"
        return block

    # ----- Operators (NEW) -----
    if cmd.kind == "operator_random":
        block = ET.Element("block", {"type": "operator_random", "id": _new_id()})
        _add_value_with_number(block, "FROM", int(cmd.value or 1))
        _add_value_with_number(block, "TO", int(cmd.value2 or 10))
        return block

    if cmd.kind == "operator_join":
        block = ET.Element("block", {"type": "operator_join", "id": _new_id()})
        _add_value_with_text(block, "STRING1", cmd.text or "")
        _add_value_with_text(block, "STRING2", cmd.text2 or "")
        return block

    if cmd.kind == "operator_contains":
        block = ET.Element("block", {"type": "operator_contains", "id": _new_id()})
        _add_value_with_text(block, "STRING1", cmd.text or "")
        _add_value_with_text(block, "STRING2", cmd.text2 or "")
        return block

    if cmd.kind == "operator_mathop":
        block = ET.Element("block", {"type": "operator_mathop", "id": _new_id()})
        field_el = ET.SubElement(block, "field", {"name": "OPERATOR"})
        field_el.text = (cmd.effect or "abs")
        _add_value_with_number(block, "NUM", int(cmd.value or 0))
        return block

    if cmd.kind == "operator_round":
        block = ET.Element("block", {"type": "operator_round", "id": _new_id()})
        _add_value_with_number(block, "NUM", int(cmd.value or 0))
        return block
